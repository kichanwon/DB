 ...
 * 
 * 
 *  view 데이터를 가지고 있지 않고 SQL문장을 가지고 있다. 
 *  따라서 사용자가 VIEW를 조회할 때 해당 SQL 문장이 실행되어 원본 테이블에 가서 데이터를 가지고 온 후 
 * 사용자에게 반환하고 데이터를 삭제하는데, 이런 특성이 대용량 VIEW일 경우에는 성능상 문제가 된다. 
 * 
 * 이럴때 MVIEW를 사용하게 된다. MVIEW는 어떤 결과를 뽑아 내는 Query가 빈번히 사용 될 경우, 
 * 데이터베이스의 한 테이블로 저장 하여 그 테이블을 조회 하도록 하는 것이다. 
 * 
 * 따라서 Query 실행 시간의 수행속도가 향상되게 된다. 실질적 데이터가 존재하는 VIEW인 것이다.
 */
...
** 이 라인은 원본 테이블에 데이터가 변경되었을 경우 MVIEW와 언제 어떻게 동기화를 시킬 것인지에 대한 옵션이며 
*ON DEMAND, ON COMMIT 옵션이 있다. 

** ON COMMIT 옵션은 원본 테이블에 데이터 변경후 COMMIT이 발생하면 자동으로 동기화 시킨다. 
*하지만 ON COMMIT 옵션은 원본 테이블에 데이터 변경이 많을 경우 동기화 시키느라 많은 부하를 발생시킬 수 있기 때문에 
*원본 테이블에 그룹 함수를 사용하거나 MVIEW에 조인이 되는 SQL만 있거나 
*또는 GROUP BY절에 사용된 컬럼에 COUNT 함수가 사용되는 경우에만 사용 가능하다.
...
SELECT SEQ_TEST_NO.currval , min_value , max_value , increment_by , cache_size , last_number
FROM all_sequences
...
/*
 * 
시퀀스 설정에 CACHE를 설정해줬다거나 아무설정을 해주지 않았을때 
기본 DEFAULT값이 20이 들어갔을 경우다. 
(20만큼 미리 오라클 내부에서 NEXTVAL이 된 상태로 존재하기때문에 LAST_NUMBER를 했을경우 그 값이 나오는 것이다.)
 */
...
INSERT INTO TB_TEST 
VALUES(SEQ_TEST_NO.NEXTVAL , 'a'); 

INSERT INTO TB_TEST 
VALUES(SEQ_TEST_NO.NEXTVAL , 'b');


SELECT * FROM TB_TEST;

SELECT rowid , tt.TEST_NO , tt.TEST_NAME
FROM TB_TEST tt;
