...
WHERE o.STATUS = 'N';
-- join 시 on 보다 using이 효과적
-- on 사용시 테이블의 모든 컬럼 조회
-- 되도록 USING 사용
...
--3
--SELECT ORDER_ID , p.PRODUCT_NAME , p.STOCK_QUANTITY , p.PRICE , (SUM(p.PRICE) OVER(ORDER BY ORDER_ID))
--FROM ORDERS o
...
SELECT 
    ...
    SUM(od.QUANTITY * p.PRICE) OVER (ORDER BY p.PRICE ) AS RUNNING_TOTAL
FROM ORDERS o
JOIN CUSTOMERS c USING (CUSTOMER_ID)
JOIN ORDER_DETAILS od USING (ORDER_ID)
JOIN PRODUCTS p USING (PRODUCT_ID)
